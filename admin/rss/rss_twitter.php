...
$channel_atom_link->setAttribute("href","http://www.markhamp23.tk/admin/rss_twitter.xml"); //url of the feed
...
$stmt=$conn->prepare("SELECT * FROM post where Permission = '1' ORDER BY data desc limit 10");
$stmt->execute();
...
$title_node = $item_node->appendChild($xml->createElement("title")); //Add Title under "item"
$title_contents = $xml->createCDATASection($row['Title']);
$title_node->appendChild($title_contents);

$link_node = $item_node->appendChild($xml->createElement("link",  "http://www.markhamp23.tk/?sec=index_input" . $a .$row['ID'])); //add link node under "item"	
	
//Unique identifier for the item (GUID)
$guid_link = $xml->createElement("guid", "http://www.markhamp23.tk/?sec=index_input" . $a .$row['ID']);
