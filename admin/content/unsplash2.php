...
<!-- MOSTRA DEL PANELL PRINCIPAL AMB EL TÍTOL DE LA SECCIÓ I LES OPCIONS A REALITZAR -->
    <div class="container-fluid">
                        ...
                        <div class="panel panel-default">
                            ...
                            <div class="panel-body">
                                <div class="dataTable_wrapper">
                                    ...
                                    <input id='title' type="text" name="title" class="form-control" placeholder="Enter the title of the post">
                                    </div>
                                    ...
                                    <table v-if='selected === true' class="table table-striped table-bordered table-hover table-responsive pull-left" id="dataTables-example1">
                                    <label v-if='selected === true'>Selected Images:</label>
      ...
      <button @click="sumaNumeros">Suma los Números</button>
                                        ...
                                        <tr v-for='(row,index) in rows' v-if='row !== null'>  
                                        <td style='text-align: center'> 
                                            <div class="img-responsive, Thumb-Box" style='text-align: center'>
                                            <img :src='row.imageurl' alt='foto' width='100px' height='50px' style="border:1px solid #a6a6a6">
                                            </div>
                                        </td> 
                                        <td>   
                                        <div><span style="padding-right:10px;" class="glyphicon glyphicon-picture"></span>{{row.description}}</div>
                                        </td> 
                                        <td>
                                        <button type="button" @click='removeRow(index,rows)' :value='row.description+row.city+row.country+row.imageown+row.link+row.imageurl' class="btn btn-default  btn-outline btn-circle"><i class="fa fa-trash fa-fw"></i></button>
                                        </td>   
                                        </tr>                  
                                    </table>                                    
       
                                <div v-for='item in arrayllista'>
                ...
                /////////////////////////////////////////////////////////////////// 
                                        print "<tr>";                 				
                                            ...
                                            // print "<a type='button' class='btn btn-default' @click='addRow(item)' :value='item.description+item.city+item.country+item.imageown+item.link+item.imageurl'> Add Image </button>";
                                            // print "<input name='Params' type='hidden' v-on:input='get_data'>";



                                            //button to send
                                            print "<div>";
                                            print "<a name='Params' type='text' class='btn btn-default' @click='addRow(item)' :value=\"item.description+'?'+item.city+'?'+item.country+'?'+item.imageown+'?'+item.link+'?'+item.imageurl\"> Add Image </button>";
                                            print "</div>";

                                            // option 1 buttontoSend
                                            // print "<div>";
                                            // print "<button name='Params' type='text' class='btn btn-default' @click='addRow(item)' :value=\"item.description+'?'+item.city+'?'+item.country+'?'+item.imageown+'?'+item.link+'?'+item.imageurl\"> Add Image </button>";
                                            // print "</div>";


                                            //option 2
                                            // print "<form action=\"./home.php?sec=unsplash&sub=new\" method=\"post\" id=\"myForm\">";
                                            // print "<input id= 'x1' name='Params' type='hidden' :value=\"JSON.stringify(arrayllista)\">";
                                            // print "<a onclick=\"$(this).closest('form').submit();\" class='btn btn-default' @click='addRow(item);' :value=\"item.description+'?'+item.city+'?'+item.country+'?'+item.imageown+'?'+item.link+'?'+item.imageurl\"> Add Image </a>";
                                            // print "</form>";

                                            // print "<form action=\"./home.php?sec=unsplash&sub=new\" method=\"post\" name='myForm' id=\"myForm\" >";
                                            // print "<input name='Params' type='hidden' :value=\"item.description+'?'+item.city+'?'+item.country+'?'+item.imageown+'?'+item.link+'?'+item.imageurl\">";
                                            // print "</form>";

                                            //print "<div class='figcap' v-bind:href=\"pageUrl +'description='+ item.description +'&city='+ item.city +'&country='+ item.country +'&imageown='+ item.imageown +'&imageurl='+ item.imageurl\"></div>";
                                            print "</figure>";
                                            print "</td>";
                                            print "<td>";
                                            print "<div>&#160;</div>";
